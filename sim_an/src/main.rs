...
fn _correct_adaptive_input_validity_test(data_struct: AllData){
    ...
    let solution:[i32;SOLUTION_SIZE] = match (NVEHICLES,NCALLS){
        (1,2) => _get_predefined_solution(vec![2,1,1,2]),
        (2,4) => _get_predefined_solution(vec![3, 3, 1, 1, 2, 2, 4, 4]),
        (3,7) => _get_predefined_solution(vec![7, 6, 3, 7, 3, 1, 1, 6, 2, 2, 4, 4, 5, 5]),
        (5,18) => _get_predefined_solution(vec![17, 14, 9, 2, 18, 17, 10, 9, 2, 16, 14, 10, 7, 16, 8, 3, 11, 8, 7, 3, 1, 1, 4, 4, 5, 5, 6, 6, 11, 12, 12, 13, 13, 15, 15, 18]),
        (7,35) => _get_predefined_solution(vec![35, 33, 31, 26, 4, 35, 33, 31, 26, 32, 9, 4, 32, 9, 30, 29, 27, 30, 27, 15, 29, 24, 15, 1, 1, 2, 2, 24, 3, 3, 5, 5, 6, 6, 7, 7, 8, 8, 10, 10, 11, 11, 12, 12, 13, 13, 14, 14, 16, 16, 17, 17, 18, 18, 19, 19, 20, 20, 21, 21, 22, 22, 23, 23, 25, 25, 28, 28, 34, 34] ),
